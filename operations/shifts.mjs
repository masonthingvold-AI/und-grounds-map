...
export async function showDayLog(target,log,{owner,act,onDone}){
 ...
 target.querySelector('form').onsubmit=async event=>{event.preventDefault();const button=event.target.querySelector('button');button.disabled=true;try{const confirmed=entries.map(({label,...e},i)=>({...e,minutes:Number(target.querySelector('#minutes-'+i).value),note:target.querySelector('#time-note-'+i).value}));if(confirmed.some(e=>!Number.isInteger(e.minutes)||e.minutes<0))throw Error('Enter whole minutes of zero or greater.');await act('time_entries_confirm',{shift_id:log.shift_id,entries:confirmed});await save(owner,'day-log',null);onDone();}catch(e){fieldError(target.querySelector('#time-error'),e);button.disabled=false;}};
}
