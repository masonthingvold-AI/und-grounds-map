...
export class LocationQueue{
 ...
 async drain(){while(this.items.length&&this.currentOwner()===this.owner){const item=this.items[0];if(item.nextAt>Date.now())return;try{const result=await this.send('location_upload',{shift_id:item.shift_id,idempotency_key:item.key,samples:[item.sample]});if(result.data.rejected)this.notify('A location sample was rejected by the server.');this.items.shift();}catch(e){if(e.retryable){item.attempts++;item.nextAt=Date.now()+backoff(item.attempts);await this.save(this.owner,'locations',this.items);return;}if(['401','GRND-401'].includes(e.code))return;this.notify(e.message);this.items.shift();}await this.save(this.owner,'locations',this.items);}}
}
