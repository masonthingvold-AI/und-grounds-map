...
export function mountShell(root, {onNavigate, onSignOut}) {
 ...
 const row = ([id,label,glyph], mobile=false) => `<a class="srow" href="#${id}" data-route="${id}" data-tip="${escape(label)}" title="${escape(label)}" aria-label="${escape(label)}" ${state.route===id?'aria-current="page"':''}>${icon(glyph)}<span class="g-label">${mobile&&id==='status'?'Status':label}</span>${['day','dispatch'].includes(id)?`<span class="badge ${(id==='day'?state.unacknowledged:state.reviewCount)>0?'live':''}">${id==='day'?state.unacknowledged||0:state.reviewCount||0}</span>`:''}</a>`;
 function person() {
 ...
 function footer() {
  return `<div class="g-foot"><a class="srow" href="#queue" data-route="queue" data-tip="${state.pending?state.pending+' pending':'Synced'}" aria-label="Saved actions, ${state.pending?state.pending+' pending':'Synced'}">${icon('cloud')}<span class="g-label">${state.pending?'Saved actions':'Synced'}</span>${state.pending?`<span class="badge live">${state.pending} pending</span>`:''}</a><a class="srow" href="#settings" data-route="settings" data-tip="Settings" aria-label="Settings">${icon('settings')}<span class="g-label">Settings</span></a><button class="g-collapse" data-action="collapse" aria-label="${compact?'Expand':'Collapse'} navigation" aria-expanded="${!compact}" data-tip="${compact?'Expand':'Collapse'} navigation">${icon(compact?'chevron-right':'chevron-left')}<span class="g-label">Collapse</span></button></div>`;
 }
 function draw() {
 ...
 root.addEventListener('click', event => {
  ...
  if(target.dataset.route){event.preventDefault();sheet.close();closeAccount();onNavigate(target.dataset.route);root.querySelector('#content').focus({preventScroll:true});return;}
