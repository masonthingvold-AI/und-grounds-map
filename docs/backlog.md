...
- Per feature, a maintenance log with dated activities: planted (with what, for beds and pots), mulched, weeded, tilled (courts), watered, deadheaded. Each entry records the employee, the time, and whether the phone was in that bed when it was logged (server-side assessment, same as tasks).
...
- The map shows each bed and court colored by "days since" the activity you pick (mulched, weeded, tilled), so what is overdue is visible at a glance.
